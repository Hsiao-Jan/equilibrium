...
func (tm *Tendermint) distributeRewards(state *state.StateDB, header *types.Header) error {
	...
	if isEpochEnding(header.Number) {
		...
		averagePrice, err = tm.oracleMgr.AveragePrice()
		if err != nil {
			return err
		}

		variables.PrevCurrencyPrice = variables.CurrencyPrice
		variables.CurrencyPrice = averagePrice
		variables

		mgr := tm.oracleMgr.Reset()

		variables.SetCurrencyPrice(averagePrice)
		va
	...
	systemVariables.PrevMintedAmount = systemVariables.PrevMintedAmount + mintedAmount
