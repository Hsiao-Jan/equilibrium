...
func NewLDBDatabase(file string, cache int, handles int) (*LDBDatabase, error) {
	...
	log.Info("Allocated cache and file handles", zap.Int("cache", cache), zap.Int("handles", handles))
...
func (db *LDBDatabase) Close() {
	...
	if db.quitChan != nil {
		...
		db.quitChan <- errc
		if err := <-errc; err != nil {
			log.Error("Metrics collection failed", zap.Error(err))
		}
		db.quitChan = nil
	}
	err := db.db.Close()
...
func (db *LDBDatabase) meter(refresh time.Duration) {
	...
	for i := 1; errc == nil && merr == nil; i++ {
		...
		if err != nil {
			log.Error("Failed to read database stats", zap.Error(err))
			merr = err
			continue
		}
		...
		for _, line := range lines {
			...
			for idx, counter := range parts[3:] {
				...
				if err != nil {
					log.Error("Compaction entry parsing failed", zap.Error(err))
					merr = err
					continue
				}
		...
		if err != nil {
			log.Error("Failed to read database write delay statistic", zap.Error(err))
			merr = err
			continue
		}
		var (
		...
		if err != nil {
			log.Error("Failed to parse delay duration", zap.Error(err))
			merr = err
			continue
		}
		...
		if err != nil {
			log.Error("Failed to read database iostats", zap.Error(err))
			merr = err
			continue
		}
		...
		if len(parts) < 2 {
			log.Error("Bad syntax of ioStats", zap.String("ioStats", ioStats))
			merr = fmt.Errorf("bad syntax of ioStats %s", ioStats)
			continue
		}
		if n, err := fmt.Sscanf(parts[0], "Read(MB):%f", &nRead); n != 1 || err != nil {
			log.Error("Bad syntax of read entry", zap.String("entry", parts[0]))
			merr = err
			continue
		}
		if n, err := fmt.Sscanf(parts[1], "Write(MB):%f", &nWrite); n != 1 || err != nil {
			log.Error("Bad syntax of write entry", zap.String("entry", parts[1]))
			merr = err
			continue
		}
