...
func TestCanonicalMappingStorage(t *testing.T) {
	...
	if entry := ReadCanonicalHash(db, number); entry != (types.Hash{}) {
...
func TestHeadStorage(t *testing.T) {
	...
	if entry := ReadHeadHeaderHash(db); entry != (types.Hash{}) {
		t.Fatalf("Non head header entry returned: %v", entry)
	}
	if entry := ReadHeadBlockHash(db); entry != (types.Hash{}) {
		t.Fatalf("Non head block entry returned: %v", entry)
	}
	if entry := ReadHeadFastBlockHash(db); entry != (types.Hash{}) {
