...
type CallMsg struct {
	...
	ComputeLimit uint64          // if 0, the call executes with near-infinite computational resource
	Amount       *big.Int        // amount of wei sent along with the call
...
// EffortEstimator wraps EstimateComputationalEffort, which tries to estimate the required
// computational effort to execute a specific transaction based on the pending state. There is no
// guarantee that this is the true required effort as other transactions may be added or
// removed by validators, but it should provide a basis for setting a reasonable default.
type EffortEstimator interface {
	EstimateComputationalEffort(ctx context.Context, call CallMsg) (uint64, error)
}
