...
func (p *StateProcessor) Process(block *types.Block, statedb *state.StateDB, cfg vm.Config) (types.Receipts, []*types.Log, uint64, error) {
	...
	for i, tx := range block.Transactions() {
		...
		receipt, _, err := ApplyTransaction(p.config, p.bc, nil, crpool, statedb, header, tx, cfg)
		if err != nil {
...
func ApplyTransaction(config *params.ChainConfig, bc ChainContext, author *common.Address, crpool *ComputationalResourcePool, statedb *state.StateDB, header *types.Header, tx *types.Transaction, cfg vm.Config) (*types.Receipt, uint64, error) {
	...
	_, effort, failed, err := ApplyMessage(env, msg, crpool)
	if err != nil {
		return nil, 0, err
	}
	...
	// @TODO (Rgeraldes) - warning: currencies
	// receipt.ComputeFee = new(big.Int).
	// receipt.StabilityFee =
