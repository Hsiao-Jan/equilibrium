...
func NewVoter(address accounts.Address, deposit *big.Int, weight *big.Int) *Voter {
	return &Voter{
		address: address,
		deposit: deposit,
		weight:  weight,
...
func (val *Voter) Address() accounts.Address { return val.address }
...
type Voters interface {
	NextProposer() *Voter
	At(i int) *Voter
	Get(addr accounts.Address) *Voter
	Len() int
	Contains(addr accounts.Address) bool
	Hash() crypto.Hash
}
...
func (voters voters) Get(addr accounts.Address) *Voter {
	for _, voter := range voters {
...
func (voters voters) Hash() crypto.Hash {
	return trie.DeriveSha(voters)
}

// Contains returns is ones Voter address is part of this set
func (voters voters) Contains(addr accounts.Address) bool {
