...
func TestMatcherWildcards(t *testing.T) {
	matcher := NewMatcher(testSectionSize, [][][]byte{
		{accounts.Address{}.Bytes(), accounts.Address{0x01}.Bytes()}, // Default address is not a wildcard
		{crypto.Hash{}.Bytes(), crypto.Hash{0x01}.Bytes()},           // Default hash is not a wildcard
		{crypto.Hash{0x01}.Bytes()},                                  // Plain rule, sanity check
		{crypto.Hash{0x01}.Bytes(), nil},                             // Wildcard suffix, drop rule
		{nil, crypto.Hash{0x01}.Bytes()},                             // Wildcard prefix, drop rule
